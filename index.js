   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : ++23490xxxxxxx")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: ++23490xxxxxx : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
//------------------------------------------------------
    XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
            await delay(1000 * 10)
            await XeonBotInc.sendMessage(XeonBotInc.user.id, { text: `🪀Support_Developer\n\n\n⎆Follow Official Channel: https://whatsapp.com/channel/0029Vaex2BtGU3BRQoeEsl0U\n\n\n` });
...
│ Yowaimo 🥳
└┬❖  
┌┤✑  Thanks using Toxxic Session Gen
│└────────────┈ ⳹        
│©2024-2099 Toxxic Md
└─────────────────┈ ⳹
